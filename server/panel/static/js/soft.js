...
function configChange(type){
	...
	switch(type){
		...
		case 'redis':
			...
			break;
		case 'subversion_add':
			fileName = '/home/svnrepos/passwd';
			break;
		case 'subversion_power':
			fileName = '/home/svnrepos/authz';
			break;
		default:
			break;
	}
...
//svn软件管理
function svnSoftMain(name, version){
	var menu = '';
	menu += '<p onclick="configChange(\'' + name + '_add' + '\')">用户管理</p>';
	menu += '<p onclick="configChange(\'' + name + '_power' + '\')">设置权限</p>';
	menu += '<p onclick="add_storage()">添加仓库</p>';
	status = false;
	$.ajax({
		url:'/plugin?action=checkSoftStart',
		data:'name=' + name,
		type:'get',
		async:false,
		success:function(rd){
	...
	layer.open({
		...
		content: '<div class="bt-w-main" style="width:640px;">\
			<div class="bt-w-menu">\
				<p class="bgw" onclick="service(\''+name+'\',\''+ status + '\')">'+lan.soft.service+'</p>'
				+menu+
			'</div>\
			<div id="webEdit-con" class="bt-w-con pd15" style="height:555px;overflow:auto">\
				<div class="soft-man-con"></div>\
...
function add_storage(act){
	if(act != undefined){
		var storageName = $("#storageName").val();
		layer.confirm("确定新建项目仓库" + storageName + "？",{closeBtn:2,icon:3},function(){
			var data = 'param='+storageName + '&name=subversion' + '&actionType=addStorage' + '&version=yum';
			var loadT = layer.msg("正在获取创建。。。",{icon:16,time:0,shade: [0.3, '#000']});
	...
	var storagePath = '<p class="conf_p">\
					<input id="storageName" class="phpUploadLimit bt-input-text mr5" style="width:350px;" type="text" name="datadir">\
					<button class="btn btn-success btn-sm" onclick="add_storage(1)">'+'创建'+'</button>\
				</p>';
	$(".soft-man-con").html(storagePath);
}
...
function AddVersion(name,ver,type,obj,title){
	...
	if(name == 'jenkins'){
		var judge = false;
		$.ajax({
			url:"/plugin?action=checkSoft",
			data:"name=tomcat",
			type:"post",
			async:false,
			success:function(rdata){
				if(!rdata){
					layer.msg('未检测到tomcat环境，请先安装tomcat',{icon:2});
					judge = true;
				}
			}
		});
