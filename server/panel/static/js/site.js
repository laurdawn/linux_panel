...
function getWeb(Path) {
	...
	$.post('/files?action=GetDir&tojs=GetFiles&p=' + p + '&showRow=' + showRow + search, data, function(rdata) {
		...
		$("#webPage").html(rdata.PAGE);
		$("#webPage div").append("<span class='Pcount-item'>每页<select style='margin-left: 3px;margin-right: 3px;border:#ddd 1px solid' class='showRow'>"+rowOption+"</select>条</span>");
		$("#webPage .Pcount").css("left","16px");
		...
		for (var i = 0; i < rdata.DIR.length; i++) {
			...
			if(getCookie("rank") == "a"){
					Body += "<tr class='folderBoxTr' data-path='" + rdata.PATH + "/" + fmp[0] + "' filetype='dir'>\
						...
						<td class='column-name'><span class='ico ico-folder'></span><a class='text' title='" + fmp[0] + fmp[5] + "'>" + cnametext + "</a></td>\
						<td>"+ToSize(fmp[1])+"</td>\
						<td>"+getLocalTime(fmp[2])+"</td>\
						<td>"+fmp[3]+"</td>\
						<td><span>\
						<a class='btlink' href='javascript:;' onclick=\"DeleteWebDir('" + rdata.PATH +"/"+ fmp[0] + "')\">"+lan.files.file_menu_del+"</a></span>\
					</td></tr>";
			}
			else{
				Body += "<div class='file folderBox menufolder' data-path='" + rdata.PATH + "/" + fmp[0] + "' filetype='dir' title='"+lan.files.file_name+"：" + fmp[0]+"&#13;"+lan.files.file_size+"：" + ToSize(fmp[1])+"&#13;"+lan.files.file_etime+"："+getLocalTime(fmp[2])+"&#13;"+lan.files.file_auth+"："+fmp[3]+"&#13;"+lan.files.file_own+"："+fmp[4]+"'>\
						...
						<div class='ico ico-folder'></div>\
						<div class='titleBox'" + fmp[0] + "</span></div>\
						</div>";
			}
		}
		for (var i = 0; i < rdata.FILES.length; i++) {
			...
			if(getCookie("rank")=="a"){
				Body += "<tr class='folderBoxTr' data-path='" + rdata.PATH +"/"+ fmp[0] + "' filetype='" + fmp[0] + "'><td><input type='checkbox' name='id' value='"+fmp[0]+"'></td>\
						...
						<td>\
						<span>\
						<a class='btlink' href='javascript:;' onclick=\"DeleteWeb('" + rdata.PATH +"/"+ fmp[0] + "')\">"+lan.files.file_menu_del+"</a>\
						</span></td></tr>";
			}
			else{
		...
		$("#setBox").click(function() {
			if ($(this).prop("checked")) {
				...
				$("#webBody > tr").addClass("ui-selected");
				
			} else {
				$("input[name=id]").prop("checked", false);
				$("#webBody > tr").removeClass("ui-selected");
			}
			showSeclect();
		});
		//阻止冒泡
		$("#webBody .btlink").click(function(e){
			e.stopPropagation();
		});
...
function DeleteWeb(fileName){
	layer.confirm(lan.get('recycle_bin_confirm',[fileName]),{title:lan.files.del_file,closeBtn:2,icon:3},function(){
		...
		$.post('/files?action=DeleteFile', 'path=' + encodeURIComponent(fileName), function(rdata) {
			...
			getWeb("/www/server/tomcat/webapps/");
		});
	});
}

//删除web目录
function DeleteWebDir(dirName){
...
function ShowRewrite(rdata){
	...
	$("#myRewrite").change(function(){
		...
		$.post('/files?action=GetFileBody','path=/www/server/panel/rewrite/'+getCookie('serverType')+'/'+rewriteName+'.conf',function(webBody){
			 $("#rewriteBody").val(webBody.data);
		});
	});
}
...
function Rewrite(siteName){
	$.post("/site?action=GetRewriteList&siteName="+siteName,function(rdata){
		...
		$.post('/files?action=GetFileBody','path='+filename,function(webBody){
			var rList = ''; 
			...
			var webBakHtml = "<div class='bt-form'>\
						...
						<textarea class='bt-input-text' style='height: 260px; width: 480px; line-height:18px;margin-top:10px;padding:5px;' id='rewriteBody'>"+webBody.data+"</textarea></div>\
			...
			$("#myRewrite").change(function(){
				...
				$.post('/files?action=GetFileBody','path='+rpath,function(webBody){
					 $("#rewriteBody").val(webBody.data);
					 editor.setValue(webBody.data);
				});
			});
		});
	});
