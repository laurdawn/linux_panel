...
echo "
+----------------------------------------------------------------------
| Linux Panel FOR CentOS/Redhat/Fedora/Ubuntu/Debian
+----------------------------------------------------------------------
| Linux Panel URL will be http://SERVER_IP:8888 when installed.
+----------------------------------------------------------------------
"
# get_node_url(){
...
while [ "$go" != 'y' ] && [ "$go" != 'n' ]
do
	read -p "Do you want to install Linux Panel to the $setup_path directory now?(y/n): " go;
done
...
path=/etc/yum.conf
isExc=`cat $path|grep mysql`
if [ "$isExc" = "" ];then
    echo "exclude=nginx mysql" >> $path
fi
...
for pace in libaio* python-devel python-imaging zip unzip openssl openssl-devel gcc libxml2 libxml2-dev libxslt* zlib zlib-devel libjpeg-devel libpng-devel libwebp libwebp-devel freetype freetype-devel lsof pcre pcre-devel vixie-cron crontabs icu libicu-devel c-ares;
do
	yum -y install ${pace}; 
done
...
Install_anaconda3()
{
	wget -O anaconda.sh https://repo.continuum.io/archive/Anaconda3-5.2.0-Linux-x86_64.sh -T 10
...
source ~/.bashrc
# #设置python3软连接
# if [ -f '/usr/bin/old_python' ];then
# 	rm -rf /usr/bin/old_python
# fi
...
cp /www/server/panel/install/bt /etc/init.d/bt 
...
if [ ! -f $setup_path/server/panel/tools.py ];then
	echo -e "\033[31mERROR: Failed to unzip, please try again!\033[0m";
	echo '============================================'
	exit;
fi
...
echo -e "=================================================================="
echo  "Linux Panel: http://$address:$port"
