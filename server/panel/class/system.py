...
class system:
    ...
    def GetConcifInfo(self,get=None):
        ...
        tmp = {}
        tmp['setup'] = os.path.exists(self.setupPath +'/redis/version.pl');
        ...
        data['systemdate'] = public.ExecShell('date +"%Y-%m-%d %H:%M:%S %Z %z"')[0].strip().decode();
    ...
    def GetDiskInfo2(self):
        #取磁盘分区信息
        # TODO 检测不到硬盘
        temp = public.ExecShell("df -h -P|grep '/'|grep -v tmpfs")[0].decode('utf-8')
        tempInodes = public.ExecShell("df -i -P|grep '/'|grep -v tmpfs")[0].decode('utf-8')
        temp1 = temp.strip().split('\n');
        tempInodes1 = tempInodes.strip().split('\n');
        diskInfo = [];
        n = 0
