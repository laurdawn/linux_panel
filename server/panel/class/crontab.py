...
class crontab:
    ...
    def checkBackup(self):
        ...
        if os.path.exists('/etc/init.d/crond'): 
            if public.ExecShell(b'/etc/init.d/crond status')[0].find(b'running') == -1: public.ExecShell(b'/etc/init.d/crond start')
        elif os.path.exists('/etc/init.d/cron'):
            if public.ExecShell(b'/etc/init.d/cron status')[0].find(b'running') == -1: public.ExecShell(b'/etc/init.d/cron start')
        elif os.path.exists('/usr/lib/systemd/system/crond.service'):
            if public.ExecShell(b'systemctl status crond')[0].find(b'running') == -1: public.ExecShell(b'systemctl start crond')
