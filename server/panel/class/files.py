...
# +-------------------------------------------------------------------
import sys,os,public,time,importlib
...
class files:
        ...
        try:
            ...
            if(get.codeing != 'byte'):
               ...
               # srcBody = srcBody.decode(char['encoding']).encode('utf-8');
                public.writeFile(filename, srcBody);
    ...
    def GetDir(self,get):
        path = get.path;
        if not os.path.exists(path): path = '/www';
        import pwd 
        dirnames = []
        filenames = []
        ...
        for filename in os.listdir(path):
            ...
            try:
                ...
                if os.path.islink(filePath): 
                    ...
                    link = ' -> ' + filePath;
                    if not os.path.exists(filePath): filePath = path + '/' + filePath;
        ...
        if hasattr(get,'disk'):
            import system
            data['DISK'] = str(system.system().GetDiskInfo());
        return data
    ...
    def GetFileBody(self,get) :
        ...
        srcBody = public.readFile(get.path).encode()
        data = {}
        data['status'] = True
        try:
            if srcBody:
    ...
    def SaveFileBody(self,get):
        ...
        try:
            ...
            public.writeFile(get.path,data);
