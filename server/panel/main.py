...
CloseLogs();
import sys,web,io,os,traceback
...
class panelPlugin(common.panelAdmin):
    ...
    def funObj(self):
        ...
        defs = ('checkSoftStart', 'checkSoft', 'flush_cache','GetCloudWarning','install','unInstall','getPluginList','getPluginInfo','getPluginStatus','setPluginStatus','a','getCloudPlugin','getConfigHtml','savePluginSort')
...
def publicObject(toObject,defs):
    ...
    for key in defs:
        if key == get.action:
            ...
            try:
                if hasattr(get,'html'):
                    return eval(bytes(fun, encoding='utf-8'))
                else:
                    t = eval(fun)
                    return public.getJson(t)
            except Exception as e:
                print('main.publicObject() exception,fun:', toObject.__class__.__name__ + '.' + key+'(get)')
                traceback.print_exc()
